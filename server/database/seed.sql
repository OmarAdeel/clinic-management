...
-- Run AFTER schema.sql. All demo accounts share the password: password123
-- bcrypt hash for "password123" (cost 10):

-- Temporarily disable triggers that auto-update sequences
-- (We set explicit IDs so FKs resolve; reset sequences at the end)
...
-- appointments (dates relative to now so dashboard looks alive)
...
-- Reset all sequences so new inserts get correct auto-increment IDs
